 ...
 * $Rev: 280 $ $Date: 2008-09-29 23:20:59 +0200 (Mo, 29 Sep 2008) $
 */
...
include_once($eqdkp->root_path.'plugins/'.$ctrt->get_data('path').'/config_data.php');
    ...
    function insert_log()
    {
        ...
        foreach($iallraids as $iraid)
        {
            if(!$iraid['donotadd'])
            {
                ...
                {
                    if(!$this->_ctrt_settings['OnlySimulate']) $db->query("INSERT INTO ".RAIDS_TABLE." (`raid_id`, `raid_name`, `raid_date`, `raid_note`, `raid_value`, `raid_added_by`) VALUES ('".mysql_real_escape_string($newraidid)."', '".mysql_real_escape_string($iraid['event'])."', '".mysql_real_escape_string($iraid['time'])."', '".mysql_real_escape_string($iraid['raidnote'])."', '".mysql_real_escape_string( number_format($iraid['dkp'], 2, '.', ''))."', 'RaidTracker (by ".mysql_real_escape_string($user->data['username']).")');");
...
// END EQdkpPlus mod
        ...
        //call plugin hook (bs cache update)
        if(!$this->_ctrt_settings['OnlySimulate']){
          $pm->do_hooks('/plugins/ctrt/index.php');
        }
