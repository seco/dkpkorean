...
if ($in->get('submit') != ""){
	...
	if ((($_FILES['dl_file']['name'] == "") && ($in->get('dl_url') == "")) || ($in->get('dl_name') == "")){
		...
		if ($_FILES['dl_file']['name'] != ""){
			...
			if($filename != ""){
				...
				switch ($upload_status) {
					...
					default:
						$file_type = strtolower(substr($filename, strrpos($filename, '.') + 1));
						$sql="INSERT INTO __downloads_links (url, name, description, category, date, user_id, permission, file_type, file_size, local_filename, mirrors, related_downloads) VALUES (
						'".$db->escape($filename)."', 
		...
		//External Link
			...
			$sql="INSERT INTO __downloads_links (url, name, description, category, date, user_id, permission, file_type, file_size, mirrors, related_downloads) VALUES (
