 ...
 * @version     $Rev: 5077 $
 * 
 * $Id: specialitems.php 5077 2009-06-13 20:39:37Z wallenium $
 */
...
if (!empty($_GET['itemname'])){
  ...
  $sql .= " AND ((ci.item_name =  '". $db->escape(urldecode($in->get('itemname')))."') OR (i.item_name =  '". $db->escape(urldecode($in->get('itemname')))."'))";
}
...
$uri_addon .= ( isset($_GET['show']) ) ? '&amp;show=' . $in->get('show') : '';
...
$eqdkp->set_vars(array(
    'page_title'    => $myISclass->GeneratePageTitle($user->lang['is_title_specialitems']),
