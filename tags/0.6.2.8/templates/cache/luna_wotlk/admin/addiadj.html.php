...
<table width="100%" border="0" cellspacing="1" cellpadding="2">
  ...
  <tr>
    <td width="350" valign="top" rowspan="4" nowrap="nowrap" class="row1">
...
<td width="100%" valign="top" class="row2"><b>' . ((isset($this->_tpldata['.'][0]['L_REASON'])) ? $this->_tpldata['.'][0]['L_REASON'] : ((isset($user->lang['REASON'])) ? $user->lang['REASON'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'REASON'))) . ' 	}')) . ':</b><br /><input type="text" name="adjustment_reason" size="35" maxlength="255" value="' . ((isset($this->_tpldata['.'][0]['ADJUSTMENT_REASON'])) ? $this->_tpldata['.'][0]['ADJUSTMENT_REASON'] : '') . '" class="input" /></td>
  ...
  <tr>
    <td width="100%" valign="top" class="row2">
	  <b>' . ((isset($this->_tpldata['.'][0]['L_DATE'])) ? $this->_tpldata['.'][0]['L_DATE'] : ((isset($user->lang['DATE'])) ? $user->lang['DATE'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'DATE'))) . ' 	}')) . ':</b> ' . ((isset($this->_tpldata['.'][0]['DATEPICK'])) ? $this->_tpldata['.'][0]['DATEPICK'] : '') . '<br /> ' . ((isset($this->_tpldata['.'][0]['FV_DATE'])) ? $this->_tpldata['.'][0]['FV_DATE'] : '') . '
    </td>
  </tr>
  <tr>
...
echo '
    <input type="submit" name="add" value="' . ((isset($this->_tpldata['.'][0]['L_ADD_ADJUSTMENT'])) ? $this->_tpldata['.'][0]['L_ADD_ADJUSTMENT'] : ((isset($user->lang['ADD_ADJUSTMENT'])) ? $user->lang['ADD_ADJUSTMENT'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'ADD_ADJUSTMENT'))) . ' 	}')) . '" class="mainoption" /> 
    <input type="reset" name="reset" value="' . ((isset($this->_tpldata['.'][0]['L_RESET'])) ? $this->_tpldata['.'][0]['L_RESET'] : ((isset($user->lang['RESET'])) ? $user->lang['RESET'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'RESET'))) . ' 	}')) . '" class="liteoption" />
    ';// ELSE
} else {
echo '
    <input type="submit" name="update" value="' . ((isset($this->_tpldata['.'][0]['L_UPDATE_ADJUSTMENT'])) ? $this->_tpldata['.'][0]['L_UPDATE_ADJUSTMENT'] : ((isset($user->lang['UPDATE_ADJUSTMENT'])) ? $user->lang['UPDATE_ADJUSTMENT'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'UPDATE_ADJUSTMENT'))) . ' 	}')) . '" class="mainoption" /> 
<input type="submit"  name="delete" value="' . ((isset($this->_tpldata['.'][0]['L_DELETE_ADJUSTMENT'])) ? $this->_tpldata['.'][0]['L_DELETE_ADJUSTMENT'] : ((isset($user->lang['DELETE_ADJUSTMENT'])) ? $user->lang['DELETE_ADJUSTMENT'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'DELETE_ADJUSTMENT'))) . ' 	}')) . '" class="liteoption" />
    ';// ENDIF
}
echo '
    </th>
