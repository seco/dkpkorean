...
{
		var item_string = document.post.select_item_name.options[document.post.select_item_name.selectedIndex].text ;	
	  
		document.post.item_name.value  = document.post.select_item_name.options[document.post.select_item_name.selectedIndex].value;	  		
	  	item_string = item_string.substring(item_string.indexOf("-")+2);	  		  			  	
	  	gameID_string = item_string.substring(item_string.lastIndexOf("-")+2);	  	  	
	  	item_string = item_string.substring(0,item_string.indexOf("-"));	  		  			  		  	
	  	
    	document.post.item_value.value = item_string.replace(",", ".");
    	document.post.game_itemid.value = gameID_string;
}
function pop_search()
{
...
echo '
    ...
    <br /><br /><b>- ' . ((isset($this->_tpldata['.'][0]['L_OR'])) ? $this->_tpldata['.'][0]['L_OR'] : ((isset($user->lang['OR'])) ? $user->lang['OR'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'OR'))) . ' 	}')) . ' -</b><br /><br />
    	<table align=left>
    		<tr><td align=left>' . ((isset($this->_tpldata['.'][0]['L_ENTER_NEW'])) ? $this->_tpldata['.'][0]['L_ENTER_NEW'] : ((isset($user->lang['ENTER_NEW'])) ? $user->lang['ENTER_NEW'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'ENTER_NEW'))) . ' 	}')) . ': </td><td align=left><input type="text" name="item_name" size="40" maxlength="255" value="' . ((isset($this->_tpldata['.'][0]['ITEM_NAME'])) ? $this->_tpldata['.'][0]['ITEM_NAME'] : '') . '" class="input" />' . ((isset($this->_tpldata['.'][0]['FV_ITEM_NAME'])) ? $this->_tpldata['.'][0]['FV_ITEM_NAME'] : '') . ' </td></tr>
    		<tr><td align=left>GameItemID: </td><td align=left><input type="text" name="game_itemid" size="10" maxlength="255" value="' . ((isset($this->_tpldata['.'][0]['GAME_ITEMID'])) ? $this->_tpldata['.'][0]['GAME_ITEMID'] : '') . '" class="input" /></td></tr>
    	</table>
    </td>
  </tr>
  <tr>
    <td width="100%" valign="top" class="row2">
    <b>' . ((isset($this->_tpldata['.'][0]['L_VALUE'])) ? $this->_tpldata['.'][0]['L_VALUE'] : ((isset($user->lang['VALUE'])) ? $user->lang['VALUE'] : '{ ' . ucfirst(strtolower(str_replace('_', ' ', 'VALUE'))) . ' 	}')) . ':</b> <input type="text" name="item_value" size="20" maxlength="20" value="' . ((isset($this->_tpldata['.'][0]['ITEM_VALUE'])) ? $this->_tpldata['.'][0]['ITEM_VALUE'] : '') . '" class="input" />' . ((isset($this->_tpldata['.'][0]['FV_ITEM_VALUE'])) ? $this->_tpldata['.'][0]['FV_ITEM_VALUE'] : '') . '
    </td>
  </tr>
  <tr>
