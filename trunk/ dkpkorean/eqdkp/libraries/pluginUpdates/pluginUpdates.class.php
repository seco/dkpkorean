 ...
 * -----------------------------------------------------------------------
 * Began:       2009
 * Date:        $Date: 2009-03-04 01:38:32 +0100 (Mi, 04 Mrz 2009) $
 * -----------------------------------------------------------------------
 ...
 * @version     $Rev: 4091 $
 * 
 * $Id: pluginUpdates.class.php 4091 2009-03-04 00:38:32Z wallenium $
 */
...
if (!class_exists("pluginUpdates")) {
  class pluginUpdates
  {
    		  ...
    		  $last_entry = '';
               ...
               if($rowcc['config_name'] == $dbfields['fieldprefix'].'inst_version'){
                  ...
                  $last_entry = max(array_keys($up_updates));
      				  ...
      				  if($last_entry != '' && $last_entry > $myvfiles['version']){
        					$tmpplugarray[$plugg]  = ($myvfiles['version']) ? $myvfiles['version'] : $user->lang['cl_unknown'];
        					$this->show_warn       = true;
      					}
    					}
    				}
      ...
      foreach($this->plugin_array as $pluginname=>$pluginversion){
  		  $sentence = sprintf($user->lang['puc_pluginneedupdate'], $user->lang[$pluginname], $pluginversion, $pm->get_data($pluginname, 'version'));
  			$out_htm .= '    	<tr>
  								    		<td>
  								    			'.$sentence.'
  								    		</td>
  								    		<td>
  								    		<a href="'.$eqdkp_root_path.'plugins/'.$pluginname.'/admin/settings.php" target="blank">'.$user->lang['puc_solve_dbissues'].'</a>
  								    		</td>
  								    		</tr>';
  		}
  		$out_htm .= '			</table>
