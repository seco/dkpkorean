 ...
 * @version     $Rev: 2560 $
 * 
 * $Id: listprofiles.php 2560 2008-08-16 23:47:56Z wallenium $
 */
    ...
    while ( $row = $db->fetch_record($members_result) )
    {
        ...
        if ( member_display($row) )
        {
            $member_count++;
             ...
             else
             {
                 $rankimg = '';
					...
					$menulink = array();
					$menulink[1] = $armoryurl.'/character-sheet.xml?r='.stripslashes(rawurlencode(utf8_encode($conf['uc_servername']))).'&n='.stripslashes(rawurlencode(utf8_encode($row['member_name'])));
					$menulink[2] = $armoryurl.'/character-talents.xml?r='.stripslashes(rawurlencode(utf8_encode($conf['uc_servername']))).'&n='.stripslashes(rawurlencode(utf8_encode($row['member_name'])));
					$menulink[3] = ($row['guild']) ? $armoryurl.'/guild-info.xml?r='.stripslashes(rawurlencode(utf8_encode($conf['uc_servername']))).'&n='.stripslashes(rawurlencode(utf8_encode($row['guild']))).'&p=1' : '';
            ...
            $tpl->assign_block_vars('members_row', array(
                ...
                'NAME_ENC'      => rawurlencode(utf8_encode($row['member_name'])),
