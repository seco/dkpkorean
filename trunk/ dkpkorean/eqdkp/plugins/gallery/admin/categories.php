...
if (isset ($_POST['cat_update_id'])){
  $title_up_query = $db->query("UPDATE __gallery_categories SET category_name = '".htmlentities(strip_tags($_POST['cat_up_name']), ENT_QUOTES)."' WHERE category_id = ".$_POST['cat_update_id']);
  $descr_up_query = $db->query("UPDATE __gallery_categories SET category_comment = '".htmlentities(strip_tags($_POST['cat_up_description']), ENT_QUOTES)."' WHERE category_id = ".$_POST['cat_update_id']);
}
...
if($_GET['delete'] != '') {
  if ($_GET['delete'] == 'all'){
    ...
    $file_list_query = $db->query("SELECT * FROM __gallery_pictures");  
	  ...
	  $del_cat_query = $db->query("SELECT * FROM __gallery_categories WHERE category_id = " . $_GET['delete']);
  	$del_cat = $db->fetch_record($del_cat_query);
    $file_list_query = $db->query("SELECT * FROM __gallery_pictures WHERE category = " . $_GET['delete']);
  }
  ...
  while ($file_list = $db->fetch_record($file_list_query)){
    ...
    $delete = $db->query("DELETE FROM __gallery_categories WHERE category_id = " . $_GET['delete']);
