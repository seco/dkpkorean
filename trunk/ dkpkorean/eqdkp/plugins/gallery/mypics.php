...
if ($_POST['pid'] != ''){
  ...
  $sec_query = $db->query("SELECT * FROM __gallery_pictures WHERE id=".$_POST['pid']);
...
if (!isset($_GET['pid'])){
  if (isset($_GET['cat_id']) AND $_GET['cat_id'] != 'all') {
  	$first_query = $db->query('SELECT * FROM __gallery_pictures WHERE category = '.$_GET['cat_id'] . ' AND user_id='.$user->data['user_id']);
  } else {
...
if ($firstpic['filename'] != '' or isset($_GET['pid'])){
  ...
  if (is_numeric($_GET['cat_id'])){
    $prev_sql = $db->query("SELECT * FROM __gallery_pictures WHERE id < ". $_GET['pid']." AND user_id = ".$user->data['user_id']." AND category=".$_GET['cat_id']);
   	while($prev = $db->fetch_record($prev_sql)){
  		$prev_pic = '<a href="mypics.php?pid='.$prev['id'].'&cat_id='.$_GET['cat_id'].'"><img src="images/leftarrow.png"></a>';
  	}
  } else {
    $prev_sql = $db->query("SELECT * FROM __gallery_pictures WHERE id < ". $_GET['pid']." AND user_id = ".$user->data['user_id']);
   	while($prev = $db->fetch_record($prev_sql)){
  ...
  if (is_numeric($_GET['cat_id'])){
    $next_sql = $db->query("SELECT * FROM __gallery_pictures WHERE id > ". $_GET['pid']." AND user_id=".$user->data['user_id']." AND category=".$_GET['cat_id']);
    $next_pid = $db->fetch_record($next_sql);
    $next_pic = '<a href="mypics.php?pid='.$next_pid['id'].'&cat_id='.$_GET['cat_id'].'"><img src="images/rightarrow.png"></a>';
  } else {
    $next_sql = $db->query("SELECT * FROM __gallery_pictures WHERE id > ". $_GET['pid']." AND user_id=".$user->data['user_id']." ORDER BY id");
    $next_pid = $db->fetch_record($next_sql);
