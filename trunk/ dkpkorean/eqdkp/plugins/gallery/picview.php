...
if (is_numeric($_GET['pid'])){
	$result = $db->query("SELECT * FROM __gallery_pictures WHERE id = '" . $_GET['pid'] . "';");
	$picture = $db->fetch_record($result);
	
	$prev_sql = $db->query("SELECT * FROM __gallery_pictures WHERE id < ". $_GET['pid']." AND category = ".$picture['category']);
	...
	$next_sql = $db->query("SELECT * FROM __gallery_pictures WHERE id > ". $_GET['pid']." AND category = ".$picture['category']);
  ...
  if ($picture['user_id'] == '-1'){
	...
	$result = $db->query("UPDATE __gallery_pictures SET views = " . $views_new . " WHERE id = " . $_GET['pid']);
