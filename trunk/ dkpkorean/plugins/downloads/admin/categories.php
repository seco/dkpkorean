	...
	if($in->get('delete') != "") {
		...
		$pdc->del_suffix('plugin.downloads');
  		if ($in->get('delete') == 'all'){
	  		$del_cat_query = $db->query("SELECT * FROM __downloads_categories");
  		...
  		while ($file_list = $db->fetch_record($file_list_query)){ 
    		$del_entry = $db->query("DELETE FROM __downloads_links WHERE id = " . $db->escape($file_list['id']));
			if (file_exists($pcache->FolderPath('downloads/').$file_list['local_filename'])) {
				unlink($pcache->FilePath('downloads/'.$file_list['local_filename']));
			};
  		}
