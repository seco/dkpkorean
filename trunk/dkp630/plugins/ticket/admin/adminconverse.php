...
function submit_reply($message, $ticket_id, $session_id, $ticketsuser_id)
{
    ...
    $worked=$db->query('INSERT INTO __ticket_replied' . $query);
    ...
    $worked1=$db->query('UPDATE __ticket_tickets SET ' . $query1 . ' WHERE ticket_id =' . $ticket_id);
    if(!$worked1){
		return array("info" => "set_replied_failed", "info_id" => $ticket_id);
	}	
	
	
	//get session ID to mark "chain tickets" also as replied
	$sql_session = 'SELECT session_id FROM __ticket_tickets WHERE ticket_id=' . $ticket_id .'';
	if (!($session_result = $db->query($sql_session))) { message_die('Could not obtain session information', '', __FILE__, __LINE__, $sql_session); }
	$sessionarray = $db->fetch_record($session_result);
	$session_id=$sessionarray['session_id'];
	
    $queryaa = $db->build_query('UPDATE', array('replied' => '1'));
    $workedaa=$db->query('UPDATE __ticket_tickets SET ' . $queryaa . ' WHERE session_id =' . $session_id . ' OR ticket_id =' . $session_id);
    if(!$workedaa){
		return array("info" => "set_replied_failed_2", "info_id" => $ticket_id);
	}
    ...
    $worked2=$db->query('UPDATE __ticket_tickets SET ' . $query2 . ' WHERE ticket_id =' . $session_id);
    if(!$worked2){
 ...
 //user-specified email config
    ...
    {
    	$sql_email = 'SELECT user_email FROM __users WHERE user_id =' . $ticketsuser_id;
	...
	//echo($email->extra_headers);
        ...
        $script_name = ( $script_name != '' ) ? $script_name . '/plugins/ticket/index.php' : 'plugins/ticket/index.php';
...
function standalone_reply($message, $tousername)
{
	...
	$sql_user_id='SELECT user_id FROM __users WHERE username="'. $tousername.'"';
...
//$fakemessage = 'Admin wants sthm';
	...
	'session_id'	=> 0,
    	...
    	$worked=$db->query('INSERT INTO __ticket_tickets' . $query);
		if(!$worked){
			return array("info" => "ticket_failed", "info_id" => $touser_id);
		}
	//--Get the just created ticket-id 
	$justticket_sql = 'SELECT ticket_id FROM __ticket_tickets WHERE user_id = ' . $touser_id .' AND message_date = ' . $timesent;
...
function delete_ticket($ticket_id)
{
	...
	$sql_deletion = 'SELECT deletion FROM __ticket_tickets WHERE ticket_id=' . $ticket_id .'';
    ...
    {
	  $sql_ticket="DELETE FROM __ticket_tickets WHERE ticket_id=" . $ticket_id;
	  if (!($ticket_result = $db->query($sql_ticket))) {
		message_die('Could not delete ticket', '', __FILE__, __LINE__, $sql); 
	  } else {
		$sql_reply="DELETE FROM __ticket_replied WHERE ticket_id=" . $ticket_id;
        ...
        $sql_get_tickets="SELECT ticket_id FROM __ticket_tickets WHERE session_id=" . $ticket_id;
        ...
        {
            $sql_del_reply="DELETE FROM __ticket_replied WHERE ticket_id=" . $tickets_id_row['ticket_id'];
            if (!($reply_del_result = $db->query($sql_del_reply))) {
			message_die('Could not delete replies to replytickets', '', __FILE__, __LINE__, $sql);
		    }
            $sql_del_replyticket="DELETE FROM __ticket_tickets WHERE ticket_id=" . $tickets_id_row['ticket_id'];
	...
	$delquery = $db->build_query('UPDATE', array('deletion' => '2', 'replied' => '1'));
	$sql_ticket="UPDATE __ticket_tickets SET ". $delquery." WHERE ticket_id=" . $ticket_id;
	//$sql_ticket="UPDATE __ticket_tickets SET deletion='2' WHERE ticket_id=" . $ticket_id;
	if (!($ticket_result = $db->query($sql_ticket))) { 
		message_die('Could not set ticket for deletion ', '', __FILE__, __LINE__, $sql_ticket); 
	} 
	$repquery = $db->build_query('UPDATE', array('replied' => '1'));
	$sql_rep_ticket="UPDATE __ticket_tickets SET ". $repquery." WHERE session_id=" . $ticket_id;
	//$sql_ticket="UPDATE __ticket_tickets SET deletion='2' WHERE ticket_id=" . $ticket_id;
	if (!($ticket_rep_result = $db->query($sql_rep_ticket))) { 
		message_die('Could not mark tickets as replied', '', __FILE__, __LINE__, $sql_rep_ticket); 
	}
	return 1;
   }
}
...
function undelete_ticket($ticket_id)
{
	...
	//register for deletion instead
	$sql_ticket="UPDATE __ticket_tickets SET deletion='0' WHERE ticket_id=" . $ticket_id;
	if (!($ticket_result = $db->query($sql_ticket))) { 
...
function mark_as_read($reply_id)
{
	...
	$sql = 'SELECT userviewed FROM __ticket_replied WHERE reply_id=' . $reply_id .'';
	...
	$newmark=($marked['userviewed']+1)%2;
    	...
    	$worked=$db->query("UPDATE __ticket_replied SET userviewed ='".$newmark."' WHERE reply_id =" . $reply_id);
	if(!$worked){
...
$tickets_total = $db->query_first('SELECT count(*) FROM __ticket_tickets');

if($tickets_total >0)
{
    $ticket_sql = 'SELECT ticket_id, session_id, user_id, message_date, message, replied, deletion
        FROM __ticket_tickets
    ...
    while ( $ticket_row = $db->fetch_record($ticket_result) )
    {
      ...
      {
      	$sql_username='SELECT username FROM __users WHERE user_id='.$ticket_row['user_id'];
	...
	$replytickets_total = $db->query_first('SELECT count(*) FROM __ticket_tickets WHERE session_id=' . $ticket_row['ticket_id']);
	//if there are replytickets get the last ticket id
	if($replytickets_total > 0)
	{
		$highestticket_sql = 'SELECT ticket_id FROM __ticket_tickets WHERE session_id=' . $ticket_row['ticket_id'] . ' ORDER BY ticket_id ASC';
	...
	//----
$repliestothisticket_total = $db->query_first('SELECT count(*) FROM __ticket_replied WHERE ticket_id=' . $ticket_row['ticket_id']);
...
(($ticket_row[replied]==0) ? '<span style="color:'.$color.'"><b>'.date($user->style['date_time'],$ticket_row['message_date']).'</b></span>'  :  date($user->style['date_time'],$ticket_row['message_date'])),
	...
	'TICKETSUSER_ID' => $ticket_row['user_id'],
        ...
        'S_REPLIES' => ($repliestothisticket_total != 0) ? true : false ,  // for coloring
        //--todo:
	...
	$ticket_id_to_reply = $ticket_row['ticket_id'];
    	...
    	if($ticket_row['replied'] != 0)
    	{
        	...
        	{
			$sql_adminname='SELECT username FROM __users WHERE user_id='.$reply_row['replied_by_id'];
	...
	$repliestothisticket2_total = $db->query_first('SELECT count(*) FROM __ticket_replied WHERE ticket_id=' . $replyticket_row['ticket_id']);


		if($repliestothisticket2_total!=0)
        	{
                    if($repliestothisticket_total!=0)
                    {
                        $showreplyheader=false;
                    }else{
                        $showreplyheader=true;
                        $repliestothisticket_total=1;
                    }
                } else {
                    $showreplyheader=false;
                }
 ...
 (($replyticket_row[replied]==0) ? '<span style="color:'.$color.'"><b>'.date($user->style['date_time'],$replyticket_row['message_date']).'</b></span>' : date($user->style['date_time'],$replyticket_row['message_date'])),
		...
		'TICKET_ID' => $replyticket_row['ticket_id'],
        	'S_REPLIES' => ($repliestothisticket2_total != 0) ? true : false ,  //for coloring
		'S_REPLYHEADER' => $showreplyheader,
