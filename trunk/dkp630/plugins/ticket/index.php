...
if (!$pm->check(PLUGIN_INSTALLED, 'ticket')) {
...
//set undeletion data - if a replyticket is submitted after an admin or a user deleted the ticket session, reset deletion to 0
    ...
    {
    	$sql_email = 'SELECT * FROM __ticket_adminemail';
	...
	// Build the server URL
        ...
        $script_name = ( $script_name != '' ) ? $script_name . '/plugins/ticket/admin/adminconverse.php' : 'plugins/ticket/admin/adminconverse.php';
...
function delete_ticket($ticket_id)
{
	...
	$delquery = $db->build_query('UPDATE', array('deletion' => '1', 'replied' => '1'));
       	$sql_ticket="UPDATE __ticket_tickets SET ".$delquery. " WHERE ticket_id=" . $ticket_id;
//       	$sql_ticket="UPDATE __ticket_tickets SET deletion='1' WHERE ticket_id=" . $ticket_id;
        if (!($ticket_result = $db->query($sql_ticket))) {
    		message_die('Could not set ticket for deletion ', '', __FILE__, __LINE__, $sql);
	    }
        //set answers to deleted deleted tickets to read
		$sql_reply="UPDATE __ticket_replied SET userviewed='1' WHERE ticket_id=" . $ticket_id;
		if (!($reply_result = $db->query($sql_reply))) { 
        ...
        while ( $tickets_id_row = $db->fetch_record($get_tickets_result) )
        {
            $sql_reply2="UPDATE __ticket_replied SET userviewed='1' WHERE ticket_id=" . $tickets_id_row['ticket_id'];
...
$tickets_total = $db->query_first('SELECT count(*) FROM __ticket_tickets WHERE user_id = ' . $user->data['user_id']);
...
if($tickets_total >0)
{
	...
	$replytickets_total = $db->query_first('SELECT count(*) FROM __ticket_tickets WHERE user_id = ' . $user->data['user_id'] .' AND session_id=' . $ticket_row['ticket_id']);
